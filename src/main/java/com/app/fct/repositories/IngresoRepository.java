...
public interface IngresoRepository extends CrudRepository<Ingreso, Integer>{
	...
	@Query(value="SELECT i.id_ingreso as idIngreso, i.fecha, i.cantidad, i.fuente, res.nombre as nombreRestaurante , r.ingresos_id_ingreso as idRestaurante FROM ingreso i join restaurante_ingresos r on r.ingresos_id_ingreso = i.id_ingreso join restaurante res on res.id=r.restaurante_id ORDER BY fecha DESC", nativeQuery=true )
